    ...
    public static class OrderDataService
    {
...

        {
            ...
            if (orderID > 0)
            {
                ...
                {
                    Console.WriteLine("Shoppings2: " + orderID+"="+ item.ProductID + "-" + item.Quantity + "-" + item.SalePrice);
        ...
        public static bool UpdateOrder(int orderID, string deliveryProvince = "", string deliveryAddress = "")
        {
            Order? data = orderDB.Get(orderID);
            if (data == null)
                return false;
