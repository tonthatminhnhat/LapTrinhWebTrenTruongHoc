...
namespace SV20T1020493.DataLayers.SQLServer
{
    public class OrderDAL : _BaseDAL, IOrderDAL
    {
        ...
        public int Add(Order data)
        {
            ...
            {
                var sql = @"insert into Orders(CustomerId, OrderTime,DeliveryProvince, 
                             DeliveryAddress,EmployeeID, Status)values(@CustomerID, getdate(),
            ...
            string searchValue = "")
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"with cte as
                            ...
                            from Orders as o WITH(INDEX(IX_OrderTime))
                            left join Shippers as s on o.ShipperID = s.ShipperID
                            left join Employees as e on o.EmployeeID = e.EmployeeID                           
                            left join Customers as c on o.CustomerID = c.CustomerID
